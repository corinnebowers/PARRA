...
```{r}
## generate one realization of the catalog with uncertainty
precip.qq <- 
  generate_precip(
...
ks.value <- 
  foreach (i = 1:n, 
    ...
    .export = 'generate_precip') %dopar% {
      ...
      # ggplot() +
      #   geom_ribbon(data = sorted,
      #     aes(ymin = p-d.crit, ymax = p+d.crit, x = precip.obs), fill = 'grey90') +
...
g4 <- ggplot(precip) + 
  ...
  annotate(geom = 'text', 
           x = casestudy$precip_mm - 20, y = 0.045, 
...
## generate probabilistic realizations of HC, soil moisture
...
## display fitted model values
model.Qp$coefficients %>% scientific(digits = 4)
  ...
  foreach (i = 1:n, 
    ...
    .export = c('generate_runoff', 'generate_hydrograph')) %dopar% {
      ...
      data.frame(
        ...
        mutate(ks = diff/d.crit) %>% 
        # arrange(desc(ks)) %>%
        # filter(Qp > quantile(catalog$Qp_m3s, 0.25)) %>% 
        pull(ks) %>% max
...
g8 <- ggplot(flow.df) + 
  ...
  # coord_cartesian(ylim = c(0, hydro.max)) + 
...
#   scale_y_origin('Average Probability of Occurrence') + 
#   coord_flip() + 
