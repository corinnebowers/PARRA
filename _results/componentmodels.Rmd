...
casestudy %>% 
  ...
  cols_label(
    ...
    IVT_max = 'Maximum IVT', duration = 'Storm Duration', 
    precip_mm = 'Total Precipitation', runoff_mm = 'Total Runoff', 
    Qp_m3s = 'Peak Streamflow', tp_hrs = 'Time to Peak Streamflow',
    sm = 'Antecedent Soil Moisture')
...
# catalog <- catalog %>% 
#   rename(runoff_obs = runoff_mm, Qp_obs = Qp_m3s)
precip <- 
  generate_precip(
    AR = AR, 
    catalog = catalog,
...
```{r}
catalog.plot <- catalog %>% 
  rename(precip_obs = precip_mm) %>% 
  mutate(precip_sim = predict.rq(model.prcp, catalog)) 
precip.max <- max(c(max(catalog.plot$precip_sim), max(catalog.plot$precip_obs)))

g3 <- ggplot(catalog.plot) + 
...
#   geom_point(aes(x = precip_obs, y = precip_obs-precip_sim)) + 
#   scale_x_origin('Storm Total Precipitation (mm)') + 
#   scale_y_continuous('Observed - Simulated \nQuantile Regression Residual (mm)') + 
#   theme(text = element_text(family = 'Segoe UI', size = 12))
# ggsave('C:/Users/cbowers/Desktop/residuals.png', height = 4)

```

```{r}
## other versions of the quantile plot

## plot single simulation (deterministic)
precip.qq <- 
  generate_precip(
...
precip.qq <- precip.qq %>% 
  ...
  left_join(catalog %>% transmute(n.AR = 1:nrow(.), IVT_max, duration, precip_obs = precip_mm))
precip.qq %>% 
  ggplot() + 
...
precip.qq %>% 
  ggplot() + 
  geom_point(aes(x = precip_obs, y = precip_sim-precip_obs)) + 
  scale_x_origin('Observed Precipitation Quantile (mm)')


## plot single simulation (probabilistic)
precip.qq <- 
  generate_precip(
    AR = catalog %>% transmute(n.AR = 1:nrow(.), IVT_max, duration), 
    catalog = catalog,
...
foreach(i = 1:10) %do% {
  precip.qq %>% 
    ...
    scale_y_origin('Simulated Precipitation Quantile (mm)') +
    geom_parity() + coord_fixed()
  ggsave(paste0('C:/Users/cbowers/Desktop/qq', i, '.png'), height = 2.5, units = 'in')
}

## plot mean of 10 simulations
precip.qq <- 
  generate_precip(
    AR = catalog %>% transmute(n.AR = 1:nrow(.), IVT_max, duration), 
    catalog = catalog,
    ...
    n.precip = 100)
precip.qq <- precip.qq %>% 
  group_by(n.AR, group = floor((n.precip-1)/10)+1) %>% 
  summarize(precip_sim = mean(precip_mm)) %>% 
  rename(n.precip = group) %>% 
  left_join(catalog %>% transmute(n.AR = 1:nrow(.), IVT_max, duration, precip_obs = precip_mm))
foreach(i = 1:10) %do% {
  precip.qq %>% 
    filter(n.precip == i) %>% 
...
precip.qq <- 
  generate_precip(
    ...
    n.precip = 1e3)
precip.qq <- precip.qq %>% 
  group_by(n.AR, group = floor((n.precip-1)/100)+1) %>% 
  summarize(precip_sim = mean(precip_mm)) %>% 
  rename(n.precip = group) %>% 
  left_join(catalog %>% transmute(n.AR = 1:nrow(.), IVT_max, duration, precip_obs = precip_mm))
foreach(i = 1:10) %do% {
...
g4 <- ggplot(precip) + 
  ...
  annotate(geom = 'text', 
           x = casestudy$precip_mm - 20, y = 0.065, 
...
sum(casestudy$precip_mm > precip$precip_mm)/nrow(precip)
...
xmax <- max(c(max(plot.sm$sm), max(plot.sm$fit)))
# ggplot(plot.sm) + 
...
caret::R2(
  ...
  catalog$Qp_m3s)
caret::R2(predict(model.Qp), catalog$Qp_m3s)
...
```{r}
catalog.plot <- catalog %>% 
  rename(precip = precip_mm, runoff = runoff_mm) %>% 
  rename(Qp_obs = Qp_m3s) %>% 
  mutate(Qp_sim = predict(model.Qp, .)) 
Qp.max <- max(c(max(catalog.plot$Qp_obs), max(catalog.plot$Qp_sim)))

g7 <- ggplot(catalog.plot) + 
...
ggplot(stage.plot) + 
  geom_rect(aes(xmin = ymd_hms('2019-02-25 12:00:00AM'), 
            ...
            fill = 'grey90', alpha = 0.25) + 
  geom_line(aes(x = dateTime, y = h.obs, linetype = 'Observed', 
...
ggplot() + 
  ...
  #   aes(x=x, y=y), fill = 'grey70', alpha = 0.5) +
...
inundation <- 
  generate_inundation(
    hydrograph = casestudy %>% 
      transmute(n.AR=1, n.precip=1, n.hc=1, n.runoff=1, n.hydro=1, Qp_m3s, tp_hrs), 
    samples = samples, 
    buildings = buildings, 
    probabilistic = TRUE, n.inun = 1e3)
stopCluster(cl)

length(attr(inundation, 'wet.bldg'))
...
buildings %>% 
  st_drop_geometry %>% 
  group_by(safety) %>% 
  summarize(num_all = length(inun),
            num_inun = Sum(inun>0),
            pct_inun = Sum(inun>0)/length(inun))
...
#   mutate(safety = factor(case_when(
...
#   facet_wrap(~factor(safety, levels = tags), nrow = 1) + 
#   scale_fill_manual(breaks = tags, values = roma.colors[3:1]) + 
#   scale_x_continuous('Simulated \nDamage Ratio\n\n', 
