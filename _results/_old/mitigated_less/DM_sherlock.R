    ...
    foreach(i = 1:n.inun, 
      ...
      .options.snow = list(progress = function(n) setTxtProgressBar(pb, n))) %dopar% {
        if (probabilistic) {
          ...
          found_ht <- unname(unlist(st_drop_geometry(buildings[wet.bldg, 'raised_m'])))*0.9 - 1
	  found_ht <- found_ht %>% cbind(5) %>% apply(1, min) %>% cbind(0) %>% apply(1, max)
...
#  ## use alternative (interpolated) set of beta dist functions  
