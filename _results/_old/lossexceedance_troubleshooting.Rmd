...
plot_loss_sim(2019, 91.6e6)
# ggsave('_figures/fig10_lossdist.png', width = 8.3, height = 5, units = 'cm')

1 - (sum(loss.sim$loss*coef > loss.est) / 1e4)
# 1 - (sum(loss.sim$loss > loss.est) / nrow(loss.sim))
max(loss.sim$loss)*coef / loss.est

```

```{r}
load(paste0('_results/slide12/stochastic/DV.Rdata'))
sum(loss.sim$loss)*coef/3200/1e6


```

```{r}
load(paste0('_results/slide9/event_', year, '/prcp4/Q.Rdata'))
load(paste0('_results/slide9/event_', year, '/prcp4/DV.Rdata'))

temp <- loss.sim %>% 
  left_join(hydrograph, by = c('n.precip')) %>%
  select(-(starts_with('n')))
ggplot(temp) + 
  geom_point(aes(x = precip_mm, y = loss*coef), alpha = 0.25) + 
  geom_vline(xintercept = casestudy$precip_mm, linetype = 'dashed') + 
  geom_hline(yintercept = loss.est, linetype = 'dashed') + 
  scale_x_origin() + scale_y_origin()
ggplot(temp) + 
  geom_point(aes(x = Qp_m3s, y = loss*coef), alpha = 0.25) + 
...
## census tracts
load(paste0('_results/slide6/event_', year, '/prcp4/DV_tract.Rdata'))
sonoma %>%
  ...
  geom_sf(data = russian %>% st_transform(6417) %>% st_crop(sonoma), size = 1) +
  scale_fill_scico(palette = 'oslo', direction = -1)

## census block groups
load(paste0('_results/slide6/event_', year, '/prcp4/DV_blockgroup.Rdata'))
...
ggsave('_figures/fig10_lossmap.png', width = 6, height = 9, units = 'cm')
# assign(paste0('map', year), g)

# ## HUC-12 watersheds
# load(paste0('_results/slide6/event_', year, '/prcp4/DV_huc12.Rdata'))
...
```{r}
# load(paste0('_results/slide6/event_1995/prcp4/DV.Rdata'))
# loss1995 <- loss.group
# load(paste0('_results/slide6/event_2006/prcp4/DV.Rdata'))
# loss2006 <- loss.group
# load(paste0('_results/slide6/event_2019/prcp4/DV_tract.Rdata'))
# loss2019 <- loss.group
# 
...
```{r}
# ## old six-panel version of figure 10
# plot_grid(
...
load(paste0('_results/stochastic/AR.Rdata'))
load(paste0('_results/stochastic/PRCP.Rdata'))
load(paste0('_results/stochastic/Q.Rdata'))
load(paste0('_results/stochastic/INUN.Rdata'))
...
g1 <- ggplot() + 
  geom_point(data = loss.merged, aes(x = precip, y = loss), 
             color = 'grey50', alpha = 0.1) + 
  geom_point(data = loss.merged %>% filter(n.AR == 89), 
             aes(x = precip, y = loss, color = '1995')) + 
  geom_point(data = loss.merged %>% filter(n.AR == 207), 
             aes(x = precip, y = loss, color = '2006')) + 
  geom_point(data = loss.merged %>% filter(n.AR == 380), 
...
g2 <- ggplot() + 
  geom_point(data = loss.merged, aes(x = Qp, y = loss), 
             color = 'grey50', alpha = 0.1) + 
  geom_point(data = loss.merged %>% filter(n.AR == 89), 
...
p <- sum(loss.sim$loss > 91.6e6)/3200
1/p

## return period of 1-in-100 year event
loss.sim %>% 
  arrange(desc(loss)) %>% 
...
loss.sim %>% 
  ...
  mutate(RP = 1/p) %>% 
  ggplot() + 
  geom_step(aes(x = loss, y = p), size = 0.75) + 
