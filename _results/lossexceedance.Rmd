...
load('_results/event/results/DV.Rdata')
...
data.frame(
  ...
  maxloss = max(loss.sim$loss)/1e6,
  scalefactor = max(loss.sim$loss) / loss.est) %>% 
  gt
...
AAL <- c('NFIP' = aal.claims/penetration)
...
load('_results/stochastic/results/DV.Rdata')
loss.stochastic <- loss.sim

## calculate AAL
AAL <- c(AAL, 'PARRA' = sum(loss.stochastic$loss)/3200)
