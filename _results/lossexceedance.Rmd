...
## Load simulation results from Sherlock
```{r}
...
## Calculate comparison AAL from Corringham et al. (2020)
```{r}
...
ggplot() + 
  ...
  geom_step(aes(x = sort(loss.stochastic$loss), y = (n:1)/(n+1)), size = 1) + 
...
## Calculate building-level losses for elevated case
```{r}
...
## Consider different prioritization schemes for building elevations
```{r}
