---
title: "Sonoma 2019 AR Case Study Demonstration"
output:
  html_document:
    toc: true 
    toc_float: true
...
ggplot() + 
  ...
  theme_void() 
ggsave('_figures/fig02/fig02_sonoma.png', width = 8.5, units = 'cm', dpi = 600)
...
bed <- raster('_scripts/5_INUN/fit_inundation/5c_run_2019event/results/casestudy_SGC_bedZ.asc', 
              crs = projection(aoi))
bed[][bed[]>1e9] <- NA
gages.elev <- gages %>%
  st_as_sf(coords = c('dec_long_va', 'dec_lat_va'), crs = 4269) %>%
...
lisflood <- raster('_scripts/5_INUN/fit_inundation/5c_run_2019event/results/casestudy.max',
                   crs = projection(aoi)) %>% 
...
More information can be found in the `5a_run_bestfit` folder.
...
Please see the script `results.Rmd` for calculations related to the next portion of the paper. 
