...
## define number of Monte Carlo realizations per variable
...
AR <- catalog %>% 
  filter(start_day == ymd('2019-02-25')) %>% 
...
## fit antecedent hydrologic conditions component model
...
if (!is.na(prob.hc)) {
  ...
  # ## use observed soil moisture
  # precip <- catalog %>% 
  #   filter(start_day == ymd('2019-02-25')) %>% 
  #   transmute(n.AR = 1, n.precip = 1, n.hc = 1, IVT_max, duration, precip_mm, sm) %>% 
  #   mutate(p = plnorm(sm, meanlog = fit.tp['meanlog'], sdlog = fit.tp['sdlog']))    
}
...
cl <- parallel::makeCluster(num_cores)
registerDoSNOW(cl)
cat('   aggregating loss by block group...\n')
loss.group <- 
  generate_losses(
    damage = damage,
    buildings = buildings %>% mutate(group = blockgroup, value.sd = acs.sd),
    aggregate = 'group',
