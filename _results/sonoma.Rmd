...
ggplot() + 
  geom_sf(data = california, fill = 'grey90', color = 'grey50', alpha = 0.5, size = 0.25) + 
  geom_sf(data = california %>% filter(NAME == 'Sonoma'),
          fill = 'grey50', color = 'grey25', alpha = 0.5, size = 0.5) +
  geom_sf(data = st_union(california), fill = NA, color = 'grey25', size = 0.75) + 
  # geom_sf(data = russian %>% st_transform(st_crs(california)), 
  #         color = baker[1], size = 0.75) +
  theme_void() 
ggsave('_figures/fig2_california.png', width = 4, units = 'cm')
