...
ggplot() + 
  ...
  annotation_scale(width_hint = 0.25, height = unit(0.25, 'cm'),
                   pad_x = unit(2.5, 'cm'), pad_y = unit(2.5, 'cm'),
                   text_family = 'Segoe UI', text_cex = 0.9) + 
  theme_void() 
ggsave('_figures/fig2_sonoma.png', height = 8)
...
ggplot() + 
  geom_sf(data = california, fill = 'grey90', color = 'grey50', alpha = 0.5) + 
  geom_sf(data = california %>% filter(NAME == 'Sonoma'),
          fill = 'grey50', color = 'grey25', alpha = 0.5, size = 1) +
  geom_sf(data = st_union(california), fill = NA, color = 'grey25', size = 1) + 
  geom_sf(data = russian %>% st_transform(st_crs(california)), 
          color = baker[1], size = 0.75) +
  theme_void() 
ggsave('_figures/fig2_california.png', height = 8)
