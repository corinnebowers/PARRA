...
buildings$elevation <- terra::extract(rast(dem), st_coordinates(buildings))
#######################################################
...
load(paste0('_results/sept30/stochastic-69/DM.Rdata'))
...
load(paste0('_results/sept30/mitigated-69/DM.Rdata'))
id.wet <- attr(damage, 'buildings') %>% as.data.frame %>% pull(id)
buildings.wet <- buildings %>% arrange(bldg) %>% 
  filter(bldg %in% id.wet) %>% 
  st_drop_geometry
...
loss.bldg <- 
  ...
  left_join(
    buildings %>% st_drop_geometry %>% 
      select(bldg, value, dist_m, elevation, order), by = 'bldg')
...
if (priority == 'loss') {		#arrange by decreasing expected loss
  loss.bldg <- loss.bldg %>% arrange(desc(loss.stochastic))
} else if (priority == 'value') {	#arrange by descreasing valuation
...
} else if (priority == 'dist') {	#arrange by increasing distance to the river
  loss.bldg <- loss.bldg %>% arrange(dist_m)
  ##########################################
} else if (priority == 'elev') {  #arrange by increasing elevation
  loss.bldg <- loss.bldg %>% arrange(elev)
} else {
