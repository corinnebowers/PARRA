...
if (!is.na(prob.prcp) | !is.na(prob.hc)) {
  ...
  precip.sm <- 
    generate_soilmoisture(
      precip = precip %>% select(-sm), 
      catalog = catalog, 
      probabilistic = prob.hc,
      n.hc = n.hc)
  ## checkpoint
  save(precip, precip.sm, file = paste0(results, 'PRCP.Rdata'))
...
precip.sm <- precip.sm %>% filter(precip_mm > 0)
