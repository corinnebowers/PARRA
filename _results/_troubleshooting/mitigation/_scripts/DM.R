  ...
  if (n.inun > 1 | n.damage > 1) {
    ...
    depth <- 
      foreach(i = 1:n.inun, 
        ...
        .export = c('toNumber', 'assign_foundations')) %dorng% {
          if (probabilistic) {
    ...
    damage <- 
      foreach (i = 1:n.inun,
        ...
        .export = c('generate_damage_deterministic', 'generate_damage_probabilistic',
                    'find_nearest', 'Min', 'Mean', 'Max')) %dorng% {
          if (probabilistic) {
            map_dfr(
              .x = 1:n.damage, 
              .f = ~generate_damage_probabilistic(
                ...
                mutate(n.inun = ifelse(is.na(attr(inundation, 'n.inun')), NA, i), 
                       n.damage = .x, bldg = attr(inundation, 'buildings')[,'id']))
          } else {
            generate_damage_deterministic(
              depth[[i]], curve, hazus, flemo, beta) %>% 
              mutate(n.inun = ifelse(is.na(attr(inundation, 'n.inun')), NA, i), 
                     n.damage = NA, bldg = attr(inundation, 'buildings')[,'id'])
          }
        }
    cat('\n')
    ...
    depth <- sweep(inun, 1, found_ht, FUN = '-') %>% 
      apply(2, function(x) ifelse(x<0, 0, x)) %>% as.matrix
    
    ## make sure basements outside of the floodplain are not being flooded
    depth[] <- ifelse(depth[]>0 & inun[]==0, 0, depth[])
    
    ## generate damage ratios
    print('converting flood depths to damage ratios...')
    if (probabilistic) {
      damage <- generate_damage_probabilistic(depth, curve, hazus, flemo, beta) %>% 
        mutate(n.damage = 1)
    } else {
      damage <- generate_damage_deterministic(depth, curve, hazus, flemo, beta) %>% 
        mutate(n.damage = NA)
    }
    damage <- damage %>% 
      mutate(n.inun = ifelse(is.na(attr(inundation, 'n.inun')), NA, 1),
             bldg = attr(inundation, 'buildings')[,'id']) %>% list
  }
