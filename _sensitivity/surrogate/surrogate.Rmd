...
```{r}
files <- paste0('_sensitivity/surrogate/sherlock_npalpha/error', 1:10, '.csv')[-6]
columns <- cols(
  X1 = col_double(),
  sim = col_double(),
  n = col_double(),
...
```{r}
error.best <- error %>% filter(n == 8 & p == 2 & alpha == 0.8)
median(error.best$RMSE)
