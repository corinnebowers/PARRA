...
vars.df <- data.frame(
  ...
  description = c('Emergent Herbaceous Wetlands', 'Woody Wetlands',
                  ...
                  'Channel Depth Parameter', 'Channel Depth Parameter', 
                  'Hydrograph Shape Parameter'),
  ...
  var.min = c(0.035, 0.045, 0.02, 0.025, 0.025, 0.05, 0.08, 0.08, 0.10, 0.023, 0.12, 
              0.06, 0.06, 0.03, 0.02, 0.015, 1, 0.69, 0.05, 0),
  var.max = c(0.12, 0.15, 0.10, 0.09, 0.07, 0.16, 0.20, 0.18, 0.20, 0.10, 0.20, 0.16, 
              0.12, 0.09, 0.05, 0.075, 80, 0.82, 0.5, 10),
  default = c(0.045, 0.07, 0.05, 0.045, 0.04, 0.05, 0.12, 0.15, 0.1, 0.03, 0.15, 
              0.12, 0.08, 0.035, 0.035, 0.035, 24, 0.76, 0.3, 4))
## note: default for edge should be 40 --> expand range
...
* go back and run LISFLOOD one final time with all best-fit parameters
* calculate \& report accuracy 
* (compare it to the best-case accuracy out of the 1000 sensitivity runs)

```{r}
# bestfit <- raster('_sensitivity/rp100/sherlock/bestfit/bestfit.max') %>% 
#   overlay(dem.hydro, fun = function(x,y) ifelse(is.na(y), NA, x))
# 
# # ## best fstat value
# # bestfit <- raster('_sensitivity/rp100/sherlock/results/rpflow41.max') %>% 
# #   overlay(dem.hydro, fun = function(x,y) ifelse(is.na(y), NA, x))
# 
# # ## best hausdorff value
# # bestfit <- raster('_sensitivity/rp100/sherlock/results/rpflow47.max') %>% 
# #   overlay(dem.hydro, fun = function(x,y) ifelse(is.na(y), NA, x))
# 
# ## calculate confusion matrix statistics
...
# 
# ## calculate Hausdorff distance
# hd <- hausdorff_dist(binary(bestfit), as.matrix(obs))
# 
# ## save metrics as dataframe
...
#   hausdorff = hd)
# 
# plot(overlay(obs, bestfit, fun = confusion))
# 
# g <- read.table('_sensitivity/rp100/sherlock/rpflow47.bdy', skip = 3) %>% 
#   filter(V1 != V1[1]) %>% 
#   mutate(V2 = (V2-V2[1])/60)
#   ggplot() + geom_line(aes(x = V2/3600, y = V1))
# plotly:: ggplotly(g)
# 
# read.table('_sensitivity/rp100/sherlock/bestfit/bestfit.bdy', skip = 3) %>% 
#   ggplot() + geom_line(aes(x = V2/3600, y = V1))
