...
# 
# ## save results
# save(parcels, bldg_sonoma, file = 'buildings/parcels.Rdata')
...
load('buildings/parcels.Rdata')

## load nonzero cells
load('nonzero/nonzero.Rdata')
...
resa <- read.csv('buildings/resa.csv')
resa[resa == '<Null>'] <- NA
...
buildings <- buildings %>% 
  ...
  left_join(acs %>% transmute(
    ...
    acs_value = acs.median, acs_sd = MOE/qnorm(0.95)), by = 'tract')

```

```{r}
#### save out ####
save(buildings, file = 'buildings/buildings.Rdata')
