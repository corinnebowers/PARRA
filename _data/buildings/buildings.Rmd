...
# 
# ## save checkpoint
# save(parcels, bldg_sonoma, file = '_data/buildings/checkpoints/buildings_parcels.Rdata')
...
# load('_data/buildings/checkpoints/buildings_parcels.Rdata')
# 
# ## load nonzero cells
...
# 
# ## save checkpoint
# save(buildings, file = '_data/buildings/checkpoints/buildings_nonzero.Rdata')
...
# ## load from file
# load('_data/buildings/checkpoints/buildings_nonzero.Rdata')
# 
...
# 
# ## save checkpoint
# save(buildings, file = '_data/buildings/checkpoints/buildings_labeled.Rdata')
...
## load from file
load('_data/buildings/checkpoints/buildings_labeled.Rdata')
...
## determine the appropriate scaling factor (scaled buildings)
coef <- buildings.plot %>%
...
# ## create a CT-level scaling factor (updated buildings)
# buildings$value <- buildings %>% 
#   st_drop_geometry %>% 
#   group_by(tract = toNumber(tract)) %>% 
#   summarize(V601TotalL = median(V601TotalL)) %>% 
#   left_join(acs %>% transmute(tract = toNumber(GEOID), acs.median), by = 'tract') %>% 
#   mutate(factor = acs.median/V601TotalL) %>% 
#   select(tract, factor) %>% 
#   right_join(buildings %>% mutate(tract = toNumber(tract)), by = 'tract') %>% 
#   mutate(value = V601TotalL*factor) %>% 
#   pull(value)
  
# ggplot() + 
#   geom_histogram(aes(x = temp), binwidth = 0.1, 
#                  color = 'black', fill = 'grey90', boundary = 0) + 
#   scale_x_origin() + scale_y_origin() + 
#   coord_cartesian(xlim = c(0,10))
# ggplot() + 
#   geom_histogram(aes(x = buildings$value/1e6), binwidth = 0.1, 
#                  color = 'black', fill = 'grey90', boundary = 0) + 
#   scale_x_origin() + scale_y_origin() + 
#   coord_cartesian(xlim = c(0,10))
# ggplot() + 
#   geom_point(aes(x = sort(temp), y = sort(buildings$value/1e6))) + 
...
## attach uncertainty information
buildings <- buildings %>% 
