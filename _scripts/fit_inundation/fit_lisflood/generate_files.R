...
## save to file
write.table(samples, file = '_scripts/fit_inundation/fit_lisflood/samples_lisflood.txt',
...
null <- foreach (i = 1:n,
  ...
  .packages = c('dplyr')) %dorng% {
    ...
    ## write out .bdy file
    bdy <- matrix(c('LISFLOOD', NA, paste0('lisflood', i), NA, length(t), 'seconds'), 
                  byrow = TRUE, ncol = 2) %>% rbind(cbind(q, t))
    write.table(bdy, 
      file = paste0('_scripts/fit_inundation/fit_lisflood/files/bdy/lisflood', i, '.bdy'), 
    ...
    bci <- data.frame(matrix(
      ...
      nrow = 2, byrow = TRUE))
    write.table(bci, 
      file = paste0('_scripts/fit_inundation/fit_lisflood/files/bci/lisflood', i, '.bci'), 
      row.names = FALSE, col.names = FALSE, quote = FALSE, sep = '\t', na = '')
  }
stopCluster(cl)
...
null <- foreach (i = 1:n, 
  ...
  .export = 'toNumber') %dorng% {
    ...
    writeRaster(lulc.dem, 
      ...
      filename = paste0('_scripts/fit_inundation/fit_lisflood/files/lulc/russian.n', i, '.asc'))
  }
stopCluster(cl)
