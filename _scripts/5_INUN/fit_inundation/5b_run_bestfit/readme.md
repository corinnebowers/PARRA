...
# 1 Generate best-fit LISFLOOD map
...
# 2 Determine final accuracy

We use the accuracy metrics from that were used for validation of their
nationwide flood hazard map and refer to their values as benchmarks of
acceptable performance. The nationwide flood hazard map generated by
First Street Foundation used the same validation procedure on a
nationwide scale, comparing LISFLOOD inundation extents against the NFHL
. Goodness of fit was assessed using the metrics shown in Table , where
$TP!= $ true positive, $FP!= $ false positive, $FN!= $ false negative,
and $TN!= $ true negative. The hit rate measures how often we correctly
predict an inundated cell and is used as a metric of underprediction
tendencies. The false alarm ratio measures how often inundation is
predicted for dry cells as a metric of overprediction. The critical
success index balances overprediction vs. underprediction, measuring how
often we are correct when either the NFHL or LISFLOOD predicts
inundation. Finally, the error bias measures tendency towards
...
and their significance can be found in Wing et al. .
    ...
    stats %>%
      t %>% as.data.frame %>% 
      rbind(c(0,0,0,0), c(1,1,1,Inf), c(1, 0, 1, 1), .) %>% 
      cbind(Case = c('Min', 'Max', 'Best', 'PARRA'), .) %>% 
      gt %>% 
      tab_header(title = 'Accuracy Statistics') %>%
      fmt_markdown(Case) %>%
      fmt_percent(c(hitrate, falsealarm, fstat), decimals = 1, drop_trailing_zeros = TRUE) %>% 
      fmt_number(bias) %>% 
      cols_label(
        Case = '',
        hitrate = 'Hit Rate', 
        falsealarm = 'False Alarm Ratio', 
        fstat = 'Critical Success Index', 
        bias = 'Error Bias') %>% 
      as_raw_html()
...
<td style="padding-top: 8px; padding-bottom: 8px; padding-left: 5px; padding-right: 5px; margin: 10px; border-top-style: solid; border-top-width: 1px; border-top-color: #D3D3D3; border-left-style: none; border-left-width: 1px; border-left-color: #D3D3D3; border-right-style: none; border-right-width: 1px; border-right-color: #D3D3D3; vertical-align: middle; overflow-x: hidden; text-align: right; font-variant-numeric: tabular-nums;">87.5&percnt;</td>
<td style="padding-top: 8px; padding-bottom: 8px; padding-left: 5px; padding-right: 5px; margin: 10px; border-top-style: solid; border-top-width: 1px; border-top-color: #D3D3D3; border-left-style: none; border-left-width: 1px; border-left-color: #D3D3D3; border-right-style: none; border-right-width: 1px; border-right-color: #D3D3D3; vertical-align: middle; overflow-x: hidden; text-align: right; font-variant-numeric: tabular-nums;">23.2&percnt;</td>
<td style="padding-top: 8px; padding-bottom: 8px; padding-left: 5px; padding-right: 5px; margin: 10px; border-top-style: solid; border-top-width: 1px; border-top-color: #D3D3D3; border-left-style: none; border-left-width: 1px; border-left-color: #D3D3D3; border-right-style: none; border-right-width: 1px; border-right-color: #D3D3D3; vertical-align: middle; overflow-x: hidden; text-align: right; font-variant-numeric: tabular-nums;">69.3&percnt;</td>
<td style="padding-top: 8px; padding-bottom: 8px; padding-left: 5px; padding-right: 5px; margin: 10px; border-top-style: solid; border-top-width: 1px; border-top-color: #D3D3D3; border-left-style: none; border-left-width: 1px; border-left-color: #D3D3D3; border-right-style: none; border-right-width: 1px; border-right-color: #D3D3D3; vertical-align: middle; overflow-x: hidden; text-align: right; font-variant-numeric: tabular-nums;">2.12</td></tr>
  </tbody>
...
upon those in across all metrics.
...
Therefore this is considered to be an acceptable fit, and we will use
the LISFLOOD environment parameters calculated here for all further
