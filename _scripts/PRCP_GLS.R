...
generate_precip <- function(
  AR, catalog, probabilistic = FALSE, n.precip = 1, mixratio = 0.9) {
  ...
  extreme.id <- catalog %>% 
    mutate(n.AR = 1:nrow(.)) %>% 
    filter(duration >= quantile(duration, mixratio) & 
             IVT_max >= quantile(IVT_max, mixratio)) %>% pull(n.AR)
...
generate_soilmoisture <- function(
  ...
  if (!('sm' %in% names(precip))) {  
    ...
    if (probabilistic) {
      ...
      precip.hc <- precip %>% mutate(sm = exp(fit.sm[1]), n.hc = NA) #%>%
        # mutate(p = 0.5)
    }
